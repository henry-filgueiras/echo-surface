...
- `scripts/build_echo_pages.py` — assembles the committed Pages mirror under `public/generated/` (stdlib-only, no subprocesses, no sim regen). Copies `lab/resonant-instrument-lab/demo/{index.html,app.js,style.css}` → `public/generated/viewer/`, mirrors a browser-whitelist subset of `runs/generated/<name>/` (`summary.json`, `topology.json`, `atlas.json`, `audio.wav`, `config.yaml`, `atlas_audio/`) → `public/generated/runs/<name>/`, copies `configs/generated/<name>.provenance.json` → `public/generated/runs/<name>/provenance.json`, and writes `public/generated/.nojekyll`. `EXAMPLES` tuple is the single source of truth for which generated scenes get mirrored; new canonical examples land both here and in `runs/generated/`. Prunes stale files in both `public/generated/viewer/` and each `public/generated/runs/<name>/` on every run so no legacy artifacts can accumulate across rebuilds. Refuses to run if the submodule is not initialized (viewer source missing) or if an example's `runs/generated/<name>/` is missing (actionable error pointing at `scripts/run_echo_pipeline.sh`).
- `public/generated/` — the committed Pages mirror that backs the live visual surface at `/generated/` on the deployed site. The Vite build workflow (`.github/workflows/deploy-pages.yml`) does **not** initialize the submodule; it just runs `npm run build`, and Vite copies `public/` → `dist/` verbatim. So every file the live viewer fetches must already be in the committed tree — hence the build-time mirror done by `scripts/build_echo_pages.py` rather than a CI-side copy. Layout: (1) `public/generated/index.html` — hand-authored landing page; (2) `public/generated/viewer/` — three-file vendored copy of the lab viewer; (3) `public/generated/runs/<name>/` — per-example artifact mirror; (4) `public/generated/.nojekyll` — safety sentinel.
- `public/generated/index.html` — hand-authored. Dark-palette single-column page with one `<section class="example">` per committed example. Each section carries: the example name (h2); the source scene path (linked to GitHub source, since `examples/` lives in the repo root and is not under `public/`); the one-sentence story (the canonical "two squares phase-lock, but the lock is brittle — perturbing a node in the contour-anchored left square cleanly breaks it" for `two_squares_one_contour`); three action buttons (`open intervention atlas` with `ablate_n1` preselected, `baseline single-run`, `top intervention (ablate n1)` — the first and third are the same URL today but are framed distinctly so a reader can jump directly to either the ranked view or the selected-intervention A/B view); and a metadata list linking `config.yaml`, `provenance.json`, `summary.json`, `atlas.json`, `topology.json`, baseline `audio.wav`, and the `atlas_audio/` intervention WAVs. Inline CSS, no external stylesheet — the page is ~130 lines, self-contained, and does not depend on Vite-processed assets so it renders identically under `npm run build`, `python -m http.server`, and raw `file://` loads.
- `tests/test_generated_pages.py` — artifact-truth smoke for the committed `public/generated/` mirror. Eight stdlib-only functions mirroring the lab's `tests/test_docs_pages.py` pattern: landing page presence + canonical `<title>EchoSurface — generated adapter examples</title>`, canonical atlas link present verbatim (`viewer/index.html?atlas=../runs/echo_two_squares/atlas.json&select=ablate_n1`), canonical baseline link present verbatim (`?summaryA=../runs/echo_two_squares/summary.json`), story-phrasing protocol compliance (asserts the shape-anchored "a node in the contour-anchored left square" phrasing is present and the forbidden "a contour-anchored node" phrasing is absent — same rule `test_adapter_canonical.test_provenance_wording_is_precise` polices on the provenance side), viewer-manifest exactness (`{index.html, app.js, style.css}`, no stale files), canonical-example bundle completeness (`summary.json, topology.json, atlas.json, audio.wav, config.yaml, provenance.json` plus audible baseline + every `atlas_audio/<id>.wav`), `.nojekyll` sentinel presence, and local-link resolution (walks every `<a href>` / `<img src>` and resolves `atlas=/summaryA=/summaryB=/topologyA=/topologyB=` query values relative to the linked HTML; `select=` excluded since it's an intervention id, not a path; `https://` cross-repo GitHub-source links skipped). Total wall-time ~0.1 s. Runnable via `lab/resonant-instrument-lab/.venv/bin/python tests/test_generated_pages.py` (reuses `numpy` from the lab venv for the WAV peak-sample check).
...
**What landed.** Four new repo paths plus three small doc updates:
- `scripts/build_echo_pages.py` — the mirror builder (see the new canon line).
- `public/generated/index.html` — hand-authored landing page.
- `public/generated/viewer/` — three-file copy of `lab/resonant-instrument-lab/demo/`.
- `public/generated/runs/echo_two_squares/` — browser-whitelist copy of the canonical run (summary/topology/atlas/audio/config/provenance + all 16 intervention WAVs).
- `public/generated/.nojekyll` sentinel.
- `tests/test_generated_pages.py` — eight-function stdlib-only smoke.
- `scripts/run_echo_pipeline.sh` — gained a trailing reminder line pointing at `python3 scripts/build_echo_pages.py`.
- `README.md` — added a "Live visual surface" row to the proof-of-wormhole table and a short refresh-bundle snippet directly below the end-to-end commands.

**Live reader path.** `https://henry-filgueiras.github.io/echo-surface/generated/` → three visible action buttons on `two_squares_one_contour` → each button opens the vendored lab viewer at `../viewer/index.html?...` against the mirrored atlas/summary. Every local link on the page was walked by `test_landing_page_local_links_resolve_on_disk` and confirmed to resolve under `public/generated/`. Verified end-to-end locally: `VITE_BASE_PATH=/echo-surface/ npm run build` produced `dist/generated/{index.html, viewer/, runs/echo_two_squares/, .nojekyll}` exactly as expected.

**CI / Pages integration** (the load-bearing call). The existing `.github/workflows/deploy-pages.yml` checks out **without** `submodules: recursive` and its only build step is `npm run build`. That means two things: (a) the lab submodule is not available at CI time, so the viewer + run artifacts *must* be committed to the EchoSurface tree before push, and (b) Vite's `publicDir` handling copies `public/` → `dist/` verbatim with no asset rewriting, so `public/generated/` maps cleanly to `<site>/generated/` without any workflow changes. I deliberately did not add a `submodules: recursive` flag to the workflow: pulling the submodule into CI would be more complex (deploy keys or a PAT on a private repo) and would let viewer drift between lab `main` and the artifact set we shipped — the mirror-before-commit pattern pins the viewer version alongside the runs it reads. Same pattern the lab's own `scripts/build_pages.py` uses in its own repo.

**Design calls worth recording.**
- **`public/generated/` vs `docs/`.** The lab lives under `docs/` because its repo's Pages is configured to serve from `/docs`. EchoSurface's Pages is served from the Vite `dist/` artifact, and `public/` is Vite's conventional static-passthrough dir — so `public/generated/` is the right location. Naming them both `docs/` would look symmetric but would either confuse Vite or require workflow changes to point Pages at a different root.
- **Commit the vendored viewer + run artifacts.** Alternatives considered: (1) teach the workflow to initialize the submodule and copy the viewer at CI time — rejected because it adds CI complexity and breaks the pinning guarantee; (2) store the viewer as a git submodule inside `public/generated/` — rejected because two submodules of the same repo at different paths is a maintenance trap; (3) fetch the viewer from the lab's Pages site at runtime — rejected because it introduces a cross-origin dependency and defeats the "no backend" scope rule. The chosen mirror-before-commit pattern is simplest and matches the lab's own approach.
- **One hand-authored page, not a generated one.** With one canonical example, a manifest + generator is premature. The landing page is ~130 lines of self-contained HTML + inline CSS; adding a second example is an in-place edit. If the example set grows past ~3, the script can grow a `render_index()` step then.
- **Provenance copied in as `provenance.json`, not `<name>.provenance.json`.** The sidecar's on-repo filename carries the example name for disambiguation under `configs/generated/`, but inside `public/generated/runs/<name>/` the path already disambiguates, and the shorter filename reads better as a link target.
- **Story phrasing policed in the smoke test.** `test_story_phrasing_is_precise` asserts the shape-anchored "a node in the contour-anchored left square" phrasing is present and the forbidden "a contour-anchored node" phrasing is absent — same wording-protocol rule `test_adapter_canonical.test_provenance_wording_is_precise` polices on the provenance side. This keeps outward-facing copy aligned with the load-bearing phrasing note in canon.

**Explicitly deferred.**
- No second canonical example (waits for the next time a new scene story actually lands).
- No screenshots on the landing page (the cards link directly into the viewer, which is the strongest first-click anyway — screenshots would be lossy duplicates).
- No per-example hero sections or "what this proves" blurbs beyond the one-sentence story (keep it narrow; the spec / provenance links are one click away).
- No integration of the EchoSurface main app's styling — the generated-examples page is a focused static artifact, not an embedded view inside the React app.
- No live-recompile path or file-watcher — `scripts/build_echo_pages.py` is explicitly invoked from `run_echo_pipeline.sh`'s closing reminder.
- No CI-side test invocation — the project has no CI-side Python test runner configured yet; the smoke test runs locally.
