...
// ---------------------------------------------------------------------------
// Scope Sigil — sacred-geometry glyphs encoding musical state
// ---------------------------------------------------------------------------
//
// When viewing the full collection (low camera zoom), each scope collapses
// into a canonical sigil: a layered sacred-geometry glyph derived from its
// musical invariants.  The mapping is:
//
//   key (tonic)      → polygon rotation angle  (C=0°, every semitone = +30°)
//   mode             → polygon type  (major = hexagon, minor = pentagon)
//   BPM              → spoke count   (<75→3, 75–104→4, 105–144→6, 145+→8)
//   scene section    → outer ring system (verse=dashed, chorus=double,
//                                          bridge=arcs, drop=triple)
//   active voices    → orbiting satellites (one per role)
//   motif density    → center-eye radius
//   beat phase       → live beat-pulse animation on center eye + rings
//
// The sigil fades out as the camera zooms into a scope (sigilWeight → 0),
// revealing the full scope ellipse + clock-emitter rendering.
// ---------------------------------------------------------------------------

/** Derive the number of radial spokes from BPM. */
const bpmToSpokeCount = (bpm: number): number => {
  if (bpm < 75) return 3;
  if (bpm < 105) return 4;
  if (bpm < 145) return 6;
  return 8;
...
const SIGIL_ZOOM_FULL = 1.0;    // at this zoom and below → sigilWeight = 1
const SIGIL_ZOOM_FADE = 2.6;    // at this zoom and above → sigilWeight = 0
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      for (const scope of scopesRef.current) {
        ...
        // Active voice roles within this scope
        const scopeActiveRoles: VoiceRole[] = [...new Set(
          simulationRef.current.loops
            .filter(l =>
              l.scopeId === scope.id &&
              now >= l.scheduledAtMs &&
              now - l.scheduledAtMs < l.loopBars * (60000 / effectiveBpm) * BEATS_PER_BAR * 6,
            )
            .map(l => l.role),
        )];

        // Motif density: fraction of phrase steps that are actual notes (0–1)
        const scopeAllLoops = simulationRef.current.loops.filter(l => l.scopeId === scope.id);
        const motifDensity = scopeAllLoops.length > 0
          ? clamp(
              scopeAllLoops.reduce((sum, l) => {
                const noteCount = l.phraseNotes.filter(n => n.kind === "note").length;
                return sum + noteCount / Math.max(l.phraseNotes.length, 1);
              }, 0) / scopeAllLoops.length,
              0,
              1,
            )
          : 0;
